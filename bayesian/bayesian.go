...
// modified LogScores() to report which words were top matches for a class
...
// NSIP addition: also return map of top 5 matching words for each class
func (c *Classifier) LogScores(document []string) (scores []float64, matches [][]MatchStruct, inx int, strict bool) {
	...
	for index, class := range c.Classes {
		...
		for _, word := range document {
			...
			if _, ok := matchesmap[word]; !ok {
				matchesmap[word] = 0.0
			}
			matchesmap[word] += math.Log(data.getWordProb(word))
		}
		matches1 := make([]MatchStruct, 0)
		for k, v := range matchesmap {
			matches1 = append(matches1, MatchStruct{Word: k, Score: v})
		}
		sort.Slice(matches1, func(i, j int) bool { return matches1[i].Score > matches1[j].Score })
		outputlen := len(matches1)
		if outputlen > 5 {
			outputlen = 5
		}
		retslice := make([]MatchStruct, outputlen)
		copy(retslice, matches1)
		scores[index] = score
		matches[index] = retslice
	}
	inx, strict = findMax(scores)
	atomic.AddInt32(&c.seen, 1)
	return scores, matches, inx, strict
}
